...
const merge = (a:Batch[],b:Batch[])=>Array.from(new Map([...a,...b].map(x=>[x._id,x])).values()).sort((x,y)=>Date.parse(x.t)-Date.parse(y.t)).slice(-200);
export function useMarket(id:string,enabled=true):MarketView {
 ...
 const priced=batches.filter(b=>b.clearing_price!==null&&Number.isFinite(b.clearing_price));
 return {book,batches,last:priced.at(-1)?.clearing_price??book?.last??null,prev:priced.at(-2)?.clearing_price??null,tick,ready,connected,error};
}
