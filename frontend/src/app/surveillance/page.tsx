...
import { Activity, ArrowRight, ArrowUpRight, CheckCircle2, ChevronRight, Download, Fingerprint, LockKeyhole, Search, ShieldAlert, ShieldCheck, Sparkles, X } from "lucide-react";
import type { AgentCall, CaseDetail, CaseStatus, SecurityCase, SecurityOverview, SecurityUser } from "@contracts/types";
import { getCase, getSecurityUser, getSecurity, reviewAgents, reviewCase } from "@/lib/api";
import { Button } from "@/components/ui/button";
import { EventFeed } from "@/components/security/event-feed";
import { Dialog } from "@/components/ui/dialog";
import { Input, Select } from "@/components/ui/input";
import { BusinessAvatar, EmptyState, categoryName } from "@/components/dashboard/shared";
...
const severity=(s:string)=>s==="high"?"bg-down/10 text-down":s==="medium"?"bg-[#fcf0da] text-[#a77726]":s==="benign"?"bg-up/10 text-up":"bg-surface text-muted-foreground";
function Badge({text}:{text:string}) {return <span className={`inline-flex rounded-md px-2 py-1 text-[10px] ${severity(text)}`}>{categoryName(text)}</span>;}
...
export default function SecurityPage() {
 ...
 {tab==="assets"&&<section className="dashboard-panel overflow-x-auto"><table className="data-table"><thead><tr><th>Business</th><th>Cases</th><th>High priority</th><th>Detections</th><th>Patterns</th><th/></tr></thead><tbody>{data.assets.filter(a=>JSON.stringify(a).toLowerCase().includes(query.toLowerCase())).map(a=><tr key={a.id}><td><Link href={`/company/${a.id}`} className="flex items-center gap-2 hover:text-accent">{a.name}<ArrowUpRight size={12}/></Link></td><td className="font-mono">{a.flags}</td><td className="font-mono text-down">{a.high}</td><td className="font-mono">{a.occurrences}{a.occurrences>1&&<span className="ml-2 soft-tag">Recurring</span>}</td><td className="text-[10px] secondary">{a.rules.map(categoryName).join(", ")}</td><td><button onClick={()=>investigate(a.id)} className="text-accent">Investigate</button></td></tr>)}</tbody></table>{!data.assets.length&&<EmptyState title="No flagged assets" description="Repeat patterns and affected businesses will be grouped here."/>}</section>}
 {tab==="users"&&<section className="dashboard-panel overflow-x-auto"><table className="data-table"><thead><tr><th>Identity</th><th>Cases</th><th>High priority</th><th>Assets</th><th>Patterns</th><th/></tr></thead><tbody>{data.users.filter(u=>JSON.stringify(u).toLowerCase().includes(query.toLowerCase())).map(u=><tr key={u.id}><td><span className="flex items-center gap-2 font-mono"><Fingerprint size={16} className="text-tint-400"/>{u.id}</span></td><td>{u.flags}</td><td className="text-down">{u.high}</td><td>{u.assets.length}</td><td className="text-[10px] secondary">{u.rules.map(categoryName).join(", ")}</td><td><button onClick={()=>void openProfile(u.id)} className="text-accent">View profile</button></td></tr>)}</tbody></table>{!data.users.length&&<EmptyState title="No users flagged" description="Identities appear here when linked to a detected pattern."/>}</section>}
