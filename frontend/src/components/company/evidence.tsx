...
export function Evidence({ facts }: { facts: FieldEvidence[] }) {
  ...
  return (
    <section aria-label="Company evidence" className="min-w-0 overflow-hidden bg-card border border-line">
      <div className="border-b border-line px-3 py-2"><Label>Reported facts & estimates</Label></div>
      <ul className="divide-y divide-hairline">
        {facts.map((fact, index) => (
          <li key={`${fact.field}:${fact.source_url}:${index}`} className="min-w-0 p-3">
            <div className="flex flex-wrap justify-between gap-2 text-[14px]">
              <span className="capitalize">{fact.field.replaceAll("_", " ")}</span>
              <span className="max-w-full font-mono tabular-nums [overflow-wrap:anywhere]">
            ...
            <blockquote className="mt-2 line-clamp-4 text-[13px] secondary [overflow-wrap:anywhere]">{fact.quote}</blockquote>
