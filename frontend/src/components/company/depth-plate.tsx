 ...
 */
export function DepthPlate({ book, tick, last, pending = 0 }: { book: Book | null; tick: number; last: number | null; pending?: number }) {
  const geo = useMemo(() => (book ? figure(book.bids, book.asks, last ?? book.last ?? null) : null), [book, last]);
  const caption = geo ? `would clear ${geo.star.p.toFixed(2)} · ${geo.star.v} sh` : "";

  return (
    <Plate id={pending > 0 ? `Book at last clear · ${pending} more sealed` : "Book at last clear"} caption={caption} className="h-full">
      {geo ? (
        <svg key={tick} viewBox={`0 0 ${geo.W} ${geo.H}`} className="block w-full h-auto" role="img" aria-label={`Demand and supply from the resting book; would clear at ${geo.star.p.toFixed(2)} on ${geo.star.v} shares`}>
          ...
          <line x1={geo.x(geo.star.p)} x2={geo.x(geo.star.p)} y1={geo.y(geo.star.v)} y2={geo.H - geo.mB} stroke="#1d1956" strokeWidth={0.75} strokeDasharray="2 2" />
