...
export function AppraisalDetails({
  ...
  useEffect(() => {
    function revealFromHash() {
      if (window.location.hash !== "#appraisal-methodology" || !disclosure.current) return;
      disclosure.current.open = true;
      requestAnimationFrame(() => {
        disclosure.current?.scrollIntoView({ behavior: "smooth", block: "start" });
      });
    }
  ...
  return (
    <details
      ...
      className="group scroll-mt-6 border-y border-line"
    >
      <summary className="flex cursor-pointer list-none items-center justify-between gap-4 py-5 [&::-webkit-details-marker]:hidden">
        <div>
          <Label className="mb-1 block">Advanced</Label>
          <h2 className="text-[24px] md:text-[28px]">Appraisal evidence and calculation</h2>
        </div>
        <span
          aria-hidden
          className="font-mono text-[18px] text-accent transition-transform group-open:rotate-45"
        >
          +
        </span>
      </summary>

      <div className="border-t border-line pb-12 pt-6">
        {risks.length ? (
          <section
            aria-labelledby="red-flags-heading"
            className="mb-8 border border-down/40 bg-down/[0.05] p-4"
          >
            <div className="flex items-center gap-2">
              <span className="border border-down/40 bg-down/[0.08] px-2 py-0.5 font-mono text-[10px] text-down">
                {risks.length}
              </span>
        ...
        {valuation ? (
          <>
            <section aria-labelledby="method-heading">
              <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
                <div>
                  <Label className="mb-1 block">Derivation</Label>
                  <h3 id="method-heading" className="text-[22px]">
                    How the estimate is combined
                  </h3>
                </div>
                <Label tracking="tight">
                  {valuation.estimates.length} valuation methods
                </Label>
              </div>

              <div className="grid border border-line md:grid-cols-4 md:divide-x md:divide-line">
                {[
                  ["1", "Normalize", "Convert each valuation to log value: mᵢ = ln(Vᵢ)."],
                  ["2", "Weight", "Give tighter estimates more weight: wᵢ = 1 / σᵢ²."],
                  ["3", "Combine", "μ = Σ(wᵢmᵢ) / Σwᵢ, then add estimator disagreement."],
                  ["4", "Range", "Value = e^μ. P20 to P80 = e^(μ ± 0.84σ)."],
                ].map(([step, title, body]) => (
                  <div key={step} className="border-b border-line p-4 last:border-b-0 md:border-b-0">
                    <Label tracking="tight">{step}</Label>
                    <div className="mt-3 text-[15px]">{title}</div>
              ...
              <div className="mt-4 grid gap-px bg-line md:grid-cols-4">
                {[
                  ["Posterior value", usd(valuation.v0, { compact: true })],
                  ["Log uncertainty", valuation.sigma.toFixed(3)],
                  ["Method disagreement", valuation.disagreement.toFixed(3)],
            ...
            <section className="mt-8" aria-labelledby="methods-heading">
              <div className="mb-3 flex items-center justify-between">
                <h3 id="methods-heading" className="text-[22px]">
                  Inputs to the appraisal
                </h3>
                <Label tracking="tight">{valuation.method.replaceAll("_", " ")}</Label>
              </div>
                    ...
                    >
                      <div>
                        ...
                        <div className="mt-1 text-[14px]">
                          {METHOD_LABELS[estimate.name] ??
                            estimate.name.replaceAll("_", " ")}
                        </div>
                      </div>
                      <div>
          ...
          <p className="text-[14px] text-muted-foreground">
            A completed appraisal is not available yet.
          </p>
        )}
