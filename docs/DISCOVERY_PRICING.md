...
`DISCOVERY_LIVE=1` plus any of `GOOGLE_PLACES_API_KEY`, `QUERIT_API_KEY`, or `XAI_API_KEY` enables live discovery. `XAI_MODEL` chooses the model through the shared `app/llm.py`. Keys are server-side only. Querit performs bounded search and page retrieval with caching, throttling, timeouts and two retries for transient failures. Grok's Responses API performs server-side web search, and its inline citations are normalized into the same evidence pipeline before Grok extracts structured facts. Google Places results are saved before slower web research completes. Source pages and model research are treated as untrusted data.
