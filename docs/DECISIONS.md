...
## 029  Sat 15:20  author: A  affects: A, D
The web offer form on `/company/{id}/bid` no longer calls `POST /companies/{cid}/offer`. Making an offer composes the same letter as `services/offers.py::compose` in the browser (`lib/offer-letter.ts`), shows it beside the panel marked Sent, and delivers nothing. The demo "Owner accepts" button went with it, since there is no server offer to accept; the acquire flow is unchanged. When a real send is wanted again, the panel calls `makeOffer` and the API needs no change. No contract or collection change.

Also on the web: a ready valuation without an `llm` estimate now triggers `POST /companies/{cid}/appraise` once per session from the company brief and shows the run as a progress bar, then refreshes. It is no longer listed as missing evidence. A signed-in visit to `/` redirects to `/overview` (`/admin` for admins).

## 030  Sat 15:55  author: A  affects: A, C, D
Winning a round now closes the deal on the web. The order ticket has a "Fill this round" control, on by default, that sets the limit to the band edge (reference plus the band percent before the first print), because a uniform-price round fills every bid at or above the clearing price and charges the clearing price. When an order placed on the bid page reports `filled_qty > 0` through `GET /markets/{id}/orders/mine`, the page moves to `/company/{id}/closing?qty&price&order&round`, which renders a Share Transfer Agreement and a Holder Representation Letter filled from the cleared order, takes a signature per paper (or Sign all), then `/company/{id}/closed` celebrates and returns to `/overview`. The acquire page links to the same closing with `whole=1`, which renders an Asset Purchase Agreement and a Bill of Sale. Papers and signatures live in the browser session only. No API, contract or collection change. Verified against the real engine with `TestClient`: an edge order fills at the ask in round 1 and in each of 8 rounds against 20 bots, and the portfolio shows the shares at the clearing price.
